local function rebuild_project(co, path)
  ...
  spinner:start_spinner("Building project...")
  ...
  vim.fn.jobstart({ "dotnet", "build", path }, {
    ...
    on_exit = function(_, code)
      if code == 0 then
        spinner:stop_spinner("✅ Build successful")
      else
        spinner:stop_spinner("❌ Build failed", vim.log.levels.ERROR)
        vim.schedule(function()
          vim.notify("dotnet build failed for: " .. path, vim.log.levels.ERROR)
        end)
      end
      coroutine.resume(co)
    end,
...
return {
  {
    ...
    config = function()
      ...
      for _, value in ipairs({ "cs" }) do
        ...
        local function validate_debugger_path(path)
          if not vim.uv.fs_stat(path) then
            vim.notify("Debugger binary not found: " .. path, vim.log.levels.ERROR)
            return false
          end
          return true
        end
        if validate_debugger_path(netcoredbg_path) then
        ...
        local BASHDB_DIR = vim.fn.expand(
          "~/.local/share/nvim//mason/packages/bash-language-server/node_modules/bash-language-server/node_modules/.bin/semver"
        )
        dap.adapters.sh = {
          type = "executable",
